export const datasOfEvent = [
    {
        nameOfEvent: "Drone +",
        desc: "Something that was used as far back as 1800s, the bot technology that finds its usage everywhere from defence to agriculture, from healthcare to surveillance. Here's to the amazing chance to bring out the Abraham Karem in you and design a drone.",
        register: (<span><a target="_blank" href='https://www.google.com/' style={{ textDecoration: "none", color: "white" }}>Register Now</a></span>),
        schedule: (<span><a target="_blank" href='...' style={{ textDecoration: "none", color: "white" }}>Problem Statement</a></span>),
        prizeMoney: "₹ 5,000",
        dateBefore: "11/03",
        coor: [{
            img: require('../../dummy.jpg'),
            nameOfEventCoordinator: "Satyam sharma",
            phoneNo: (<span><a href="tel:7700076418" style={{ textDecoration: "none", color: "white" }}> 7700076418</a></span>),
            mail: (<span><a href='mailto:satyamsharma3002@gmail.com' style={{ textDecoration: "none", color: "white" }}>satyamsharma3002@gmail.com</a></span>)
        }]
    },
    {
        nameOfEvent: "Margdarshak",
        desc: "Suprised by how the seller packed it all way too fast? Well, he might have had a Line following robot. LFR, a robotic technology comprising of a bot following an arena. And what's even a thrill, you can design it too. Participate in our event Margdarshak and win worth cash prizes.",
        register: (<span><a target="_blank" href='https://www.wikipedia.com/' style={{ textDecoration: "none", color: "white" }}>Register Now</a></span>),
        schedule: (<span><a target="_blank" href='....' style={{ textDecoration: "none", color: "white" }}>Problem Statement</a></span>),
        prizeMoney: "₹ 4,000",
        dateBefore: "15/03",
        coor: [{
            img: require('../../dummy.jpg'),
            nameOfEventCoordinator: "Aarushi Jain",
            phoneNo: (<span><a href="tel:9352148289" style={{ textDecoration: "none", color: "white" }}> 9352148289 </a></span>),
            mail: (<span><a href='mailto:aarushisanghi@gmail.com' style={{ textDecoration: "none", color: "white" }}>aarushisanghi@gmail.com</a></span>)
        }]
    },
    {
        nameOfEvent: "Reconnaissance",
        desc: "Birds can't swim and fishes can't fly, isn't it? There's nothing to worry about this for there's a robotic technology that covers it all. Yes, you got it right. Hovercraft, the amphibious bot. Participate in our event, design a hovercraft and avail worth cash prizes.",
        register: (<span><a target="_blank" href='https://www.google.com/' style={{ textDecoration: "none", color: "white" }}>Register Now</a></span>),
        schedule: (<span><a target="_blank" href='....' style={{ textDecoration: "none", color: "white" }}>Problem Statement</a></span>),
        prizeMoney: "₹ 8,000",
        dateBefore: "1/03",
        coor: [{
            img: require('../../dummy.jpg'),
            nameOfEventCoordinator: "Rishav Ranjan",
            phoneNo: (<span><a href="tel:7319956071" style={{ textDecoration: "none", color: "white" }}> 7319956071</a></span>),
            mail: (<span><a href='mailto:2236075@sliet.ac.in' style={{ textDecoration: "none", color: "white" }}>2236075@sliet.ac.in</a></span>)
        }]
    },
    {
        nameOfEvent: "Robowar",
        desc: "Here's to the event that's second to none- ROBOWAR. The combat robotics technology that will make you familiar to the core of robotics, that will make you learn exclusively for it's not only about you, but the opponents' team as well. Participate in our event Robowar and win worth cash prizes.",
        register: (<span><a target="_blank" href='https://www.google.com/' style={{ textDecoration: "none", color: "white" }}>Register Now</a></span>),
        schedule: (<span><a target="_blank" href='....' style={{ textDecoration: "none", color: "white" }}>Problem Statement</a></span>),
        prizeMoney: "₹ 8,000",
        dateBefore: "1/03",
        coor: [{
            img: require('../../dummy.jpg'),
            nameOfEventCoordinator: "Indra Kumar",
            phoneNo: (<span><a href="tel:9548304247" style={{ textDecoration: "none", color: "white" }}> 9548304247 </a></span>),
            mail: (<span><a href='mailto:nishupanchal466@gmail.com' style={{ textDecoration: "none", color: "white" }}>nishupanchal466@gmail.com</a></span>)
        }]
    }
]


export const datasOfEventCoordinator = [
    {
        photoOfFaculty: require('../../dummy.jpg'),
        nameOfFaculty: "Name of Faculty",
        facultyAdvisor: "Faculty Advisor",
        domainCoordinator: [
            {
                photoOfdomainCoordinator: require('../../dummy.jpg'),
                nameOfDomainCoordinator: "Tushar Chandra",
                phoneNoOfDomainCoordinator: (<span><a href="tel:9113444651" style={{ textDecoration: "none", color: "white" }}> 9113444651 </a></span>),
                mailOfDomainCoordinator: (<span><a href='mailto: 2040425@sliet.ac.in' > 2040425@sliet.ac.in</a></span>)
            }
        ]
    }
]